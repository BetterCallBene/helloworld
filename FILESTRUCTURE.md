...
* C++ source files:
  * `include/helloworld/helloworld.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/helloworld.cpp` is the main file that implements this library.
  * `app/helloworld_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/helloworld_t.cpp` contains the unit tests for the library.
